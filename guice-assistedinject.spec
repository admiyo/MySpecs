...
%build
javac -d classes -cp src/:%{_javadir}/guice.jar `find . -name *.java` 
javadoc -d javadoc -classpath src/:%{_javadir}/guice.jar  $(for JAVA in `find src/ -name *.java` ; do  dirname $JAVA ; done | sort -u  | sed -e 's!src.!!'  -e 's!/!.!g'  )
...
%add_to_maven_depmap com.google.inject.extensions guice-assisted-inject %{version} JPP %{name}
